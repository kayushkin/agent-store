...
//
//	go test -run TestTheLiveProcessEnvironmentBuildsARegistry ./internal/config -args -live-environment-file=/proc/<pid>/environ
//
// deploy.sh runs it against the running service before it stops it, because a
// set owned AGENT_STORE_ variable that nobody declared stops the new binary at boot.
...
func TestTheLiveProcessEnvironmentBuildsARegistry(t *testing.T) {
	...
	// New's error quotes a value it cannot parse. The only setting here that
	// is not a string is the scan interval, and no setting here is a secret.
	if _, err := NewServerSettingsRegistry(servicesettings.MapEnvironment(variables)); err != nil {
